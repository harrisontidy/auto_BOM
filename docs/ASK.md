...
Common circuits first use generic declarative blueprints: buck/boost, fixed or adjustable linear regulator, inverting/non-inverting op-amp, astable timer, and digital-IC support. The selected model uses low reasoning (when supported) to supply exact pin roles, reference values, required passive additions and datasheet equations. Code expands the graph, evaluates dependency-ordered equations, selects E24/E96 values, checks units and full pin coverage, and routes connections. It never executes generated application code. The fast path uses PDF text without a separate AI page-selection or image-rendering stage. Unsupported or invalid adapters fall back to the original full-text/image workflow with the selected reasoning effort. The fast adaptation attempt is bounded to 45 seconds.

Validated adapters can be reused for an identical part/request for 24 hours across restarts, with at most 200 saved entries. The key also includes datasheet URL, blueprint definitions, symbol ID and pin map. Every reuse recalculates, validates and reroutes against current symbol geometry. A changed operating requirement creates a different key; no changed-voltage request reuses a completed old-voltage circuit. APPLICATION_FAST_PATH=false disables this path; APPLICATION_CACHE=false enables cold testing; APPLICATION_CACHE_DIR selects an isolated store. APPLICATION_FAST_MODEL is an optional internal benchmark override; the selected model remains the default. See [measured results](application-fast-path-benchmark-2026-09-20.md): fresh examples took 13–44 seconds, identical saved recipes about 0.5–0.6 seconds. Neither five-second cold generation nor 80% circuit coverage has been established.
...
Datasheet downloads identify the client as AutoBOM. A live comparison of the TD1837 catalog PDF returned HTTP 403 without a User-Agent and HTTP 200 with this honest application identifier. If a catalog URL still fails, LCSC candidates try their JLCPCB product page's published manufacturer link and the current LCSC product page's exact C-number PDF before invoking AI web search. Product-page parsing excludes certification and related-part PDFs. Public HTTPS/address validation, size limits and cancellation apply to every destination. Live downloads succeeded for TD1837 and TPS62160DSGR, including the JLCPCB-to-TI route; site availability may still change.
...
Routine missing preferences use documented defaults or explicitly stated reference operating assumptions. Only an essential unresolved input or an actual operating conflict should block a draft. Expandable design notes show a short assumption/limitation summary; full calculations stay in the circuit record and component fields; ordinary replies omit repetitive heat/ripple/layout review boilerplate unless asked. Actual known conflicts are still reported. Requests with operating requirements bypass fixed reference fast paths so the general generator checks those requirements. Both suppliers share this logic.


### Compact routing and Fast mode

The service recognizes buck topology from its connections and places input bypass, bootstrap, catch diode, inductor, output capacitor and divider in a compact arrangement. Other single-unit circuits place supporting parts near the primary pins they serve. A bounded 25 mil orthogonal router avoids symbol bodies, NC pins and unrelated nets; it preserves T junctions and collapses straight segments. If a branch is obstructed, the router first expands its search area, then uses matching labels on that branch and the wired portion of the net. Successful wires remain in place. Invalid pin geometry or conflicting pin assignments still stop placement. Native patch 0023 checks the actual loaded symbol pin positions against the routing geometry before adding items. Save and reopen the editor after installing the patch. Backend tests check full connectivity including unintended joins and NCs; KiCad CLI netlists for TPS5430 and NE555 matched their source circuits. Interactive placement was not exercised because the user requested background-only work.
