...
import { reviewCandidates } from "./services/ai.js";
...
createServer(async (request, response) => {
  try {
    if (request.method === "GET" && request.url === "/api/status") {
      return sendJson(response, 200, { digikey: Boolean(process.env.DIGIKEY_CLIENT_ID && process.env.DIGIKEY_CLIENT_SECRET), openai: Boolean(process.env.OPENAI_API_KEY) });
    }
    if (request.method === "POST" && request.url === "/api/digikey/search") {
      const { component } = await readBody(request);
      if (!component?.value) return sendJson(response, 400, { error: "A component value is required." });
