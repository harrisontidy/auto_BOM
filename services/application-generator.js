...
export function compileApplication(plan,candidate,assets,document) {
  ...
  return {id:'datasheet-application-v1',title:plan.title,description:plan.description,
    ...
    notes:[...(plan.conditions||[]),...(plan.assumptions||[]).map(a=>'Assumed: '+a),...calculations.map(c=>c.name+': '+Number(c.result.toPrecision(6))+' '+c.unit+' (PDF page '+c.page+').'),...(plan.notes||[]),'AI-designed draft: review against the linked datasheet before use. Matching net labels are electrically connected. Support footprints and part numbers remain for AutoBOM.'].join(' '),
...
async function structured(input,settings,environment,signal,images=[]) {
  if(settings.provider==='codex')return codexStructuredResponse({...input,images,model:settings.model,effort:settings.effort,signal,timeoutMs:140000},environment);
...
export async function generateApplication(input,environment=process.env,{signal,onProgress}={}) {
  ...
  const assets=await resolveKiCadAssets(component,candidate,environment);
  ...
  const settings={provider:input.provider,...selectModel(await askModels(input.provider,environment),input.model||'latest',input.effort||'auto')};
  try {
    const downloaded=await resolveCandidateDatasheet(candidate,{signal,
    ...
    const document=await readApplicationDocument(url,{signal,downloaded,selectPages:async pages=>{
      const choice=await structured({schema:object({pages:list({type:'integer'})}),
    ...
    let plan=await structured({schema:applicationSchema,instructions:applicationInstructions,
    ...
    try {circuit=compileApplication(plan,candidate,assets,document);}
      ...
      plan=await structured({schema:applicationSchema,instructions:applicationInstructions,
    ...
    circuit=await layoutApplication(circuit,assets,environment);
    return response(`Generated a datasheet-based draft for ${candidate.manufacturerPartNumber}. Click Place application circuit to add it. Calculations and assumptions are in the design notes.`,circuit,assets);
