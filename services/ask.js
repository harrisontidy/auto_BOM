...
export function shortlistAskResult(result) {
  ...
  const strong = selected && !result.pending && !selected.reviewPending
    && review.confidence >= 0.8 && !(review.concerns || []).length
...
export function createAsk({plan = planAsk, search = searchComponents} = {}) {
  return async (raw, environment = process.env, options = {}) => {
    ...
    const component = {...response, summary: (response.searchTerms || input.query).slice(0,120), pinCount: 0};
    ...
    const runSearch = interpreted => search({...input, query: response.query}, env, {...options, interpreted,
      ...(input.provider === 'codex' ? {review: (component,candidates) => codexStructuredResponse({
        ...
        instructions: 'Review the supplied supplier candidates against every explicit requirement. Use only the supplied evidence, not assumptions or remembered specifications. Missing attributes remain unknown. Return a supplierPartNumber only if the candidate plausibly meets ALL explicit requirements. Return an empty selectedSupplierPartNumber when none qualifies, including when a defining requirement such as full color is unverified. Never select a known mismatch as the closest match. Identify the unmet requirement concisely. Do not claim complete compatibility or verified CAD without evidence. Keep reasoning to one short sentence (under 30 words), without repeating stock, price or catalog specifications. Keep each concern brief and specific. Confidence must reflect the available evidence. Do not use tools or change anything.',
